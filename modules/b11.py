...
# ── NÂNG CẤP KỸ THUẬT ──────────────────────────────────────
    st.markdown("---")
    st.markdown("### 🔬 Nâng cấp: Q-table vs DQN · Learning Curves · Policy Heatmap")
    
    import os
    img_path = os.path.join(os.path.dirname(__file__),
                            "b12_aideom_vn", "b11_enhanced.png")
    if os.path.exists(img_path):
        st.image(img_path,
                 caption="Q-table vs DQN · Learning Curves · Policy Heatmap · Performance Boxplot",
                 use_container_width=True)
    else:
        if st.button("▶ Tạo biểu đồ nâng cấp (Q-learning + DQN)"):
            import subprocess, sys
            script = os.path.join(os.path.dirname(__file__),
                                  "b12_aideom_vn", "b11_enhanced.py")
